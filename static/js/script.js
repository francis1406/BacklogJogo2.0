...
function dragInto(e) {
    if (movingCard.parentNode.parentNode !== this) {
        ...
        if (this.getElementsByClassName('column over') && this.getElementById('jogando')) {
            var form = document.getElementsByClassName('card');
            form.method = 'post';
            form.action = "{{ url_for('teste') }} ";
